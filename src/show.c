...
#define SHOW_ARCHIVE_FIELDS_COUNT 10
	...
	for (i = 0; i < parray_num(actual_tli_list); i++)
	{
		...
		/* N files */
		snprintf(row->n_segments, lengthof(row->n_segments), UINT64_FORMAT,
				 tlinfo->n_xlog_files);
		...
		/* N backups */
		snprintf(row->n_backups, lengthof(row->n_backups), UINT64_FORMAT,
				  ...
				  parray *tli_list)
{
	...
	for (i = parray_num(actual_tli_list) - 1; i >= 0; i--)
	{
		...
		appendPQExpBuffer(buf, UINT64_FORMAT, tlinfo->n_xlog_files);

		json_add_key(buf, "size", json_level);
		appendPQExpBuffer(buf, UINT64_FORMAT, tlinfo->size);
